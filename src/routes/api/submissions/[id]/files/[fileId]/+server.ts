 ...
 * Moderator-only: the caller decrypts client-side using the project private key.
 */
export const GET: RequestHandler = async ({ params, locals }) => {
	if (!locals.member) throw error(401, 'Authentication required');
	if (locals.member.role !== 'MODERATOR') throw error(403, 'Only moderators can download files');

	const file = await db.submissionFile.findUnique({
		where: { id: params.fileId },
		include: { submission: { select: { projectId: true, id: true } } }
	});
